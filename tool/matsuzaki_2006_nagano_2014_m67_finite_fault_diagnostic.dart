...
const _targetEventId = '2014112222081790-36.6928-137.8910';

const _naganoM67Fault = Matsuzaki2006RectangularFaultPlane.fromPointOnPlane(
...
void main(List<String> arguments) {
  ...
  final outputDirectory = Directory(
    _value(arguments, '--output-dir') ??
        '.dart_tool/matsuzaki_2006_nagano_2014_m67_finite_fault_diagnostic',
  );
  if (inputPath == null || selectionPath == null) {
    stderr.writeln(
      'Usage: dart run '
      'tool/matsuzaki_2006_nagano_2014_m67_finite_fault_diagnostic.dart '
      '--input <2014.json> --selection <frozen-selection.json> '
  ...
  if (dataset['year'] != 2014) {
    throw FormatException('Expected JMA annual year 2014 in $inputPath.');
  }
  ...
  if (nearStations.length != 15) {
    throw StateError(
      'Expected the audited count of 15 point-source nearfield stations, '
  ...
  for (final station in nearStations) {
    ...
    final faultDistance =
        Matsuzaki2006FiniteFaultGeometry.shortestDistanceToFaultUnion(
          faultPlanes: const [_naganoM67Fault],
  ...
  final report = <String, Object?>{
    'schemaVersion':
        'matsuzaki_2006_nagano_2014_m67_finite_fault_diagnostic_v1',
    'event': {
    ...
    'sources': {
      ...
      'jmaSourceProcessReport':
          'references/papers/2014_JMA_Northern_Nagano_M67_source_process.pdf',
      'jmaSourceProcessData':
          'references/papers/2014_JMA_Northern_Nagano_M67_source_process_data.zip',
    },
    'faultParameterSemantics': {
      'reportedFaultPlaneCount': 1,
      'reportedStrikeDegrees': 25,
      'reportedDipDegrees': 61,
      'reportedRakeDegrees': 59,
      'reportedSubfaultLengthKm': 3,
      'reportedSubfaultWidthKm': 3,
      'assumedAlongStrikeSubfaultCount': 11,
      'assumedDownDipSubfaultCount': 6,
      'activeMarkValue': 1,
      'activeAlongStrikeSubfaultRange': {'first': 5, 'last': 11},
      'activeDownDipSubfaultRange': {'first': 1, 'last': 4},
      'activeAlongStrikeSubfaultCount': 7,
      'activeDownDipSubfaultCount': 4,
      'activeLengthKm': 21,
      'activeWidthKm': 12,
      'inactiveAssumedSubfaultsIncludedInDistance': false,
      'reportedPdfApproximateLengthKm': 20,
      'reportedPdfApproximateWidthKm': 15,
      'numericBoundaryBasis':
          'exact_contiguous_mark_1_subfault_rectangle_in_revised_jma_data',
      'reportedRuptureStartSubfault': {
        'alongStrikeNumber': 7,
        'downDipNumber': 2,
      },
      'ruptureStartWithinActiveRectangle': {
        'alongStrikeKmFromActiveUpperEdgeStart': 7.5,
        'downDipKmFromActiveUpperEdge': 4.5,
        'basis':
            'centre_of_Xorg_7_Worg_2_relative_to_active_X_5_to_11_W_1_to_4_grid',
      },
      'reportedRuptureSpeedKmPerSecond': 2,
      'downDipAzimuthRule': 'reported_strike_plus_90_degrees_right_hand_rule',
      'catalogDepthAndRuptureStartDepthAreDistinct': false,
    },
    'faultPlane': _naganoM67Fault.toJson(),
    'distanceSummaries': {
...
String _toMarkdown({
  ...
  final faultPlane = _naganoM67Fault.toJson();
  final buffer = StringBuffer()
    ..writeln(
      '# Matsuzaki 2006 Northern Nagano 2014 M6.7 Finite-Fault Diagnostic',
    )
    ..writeln()
    ..writeln(
    ...
    ..writeln('- Active rectangle: 21 km x 12 km, strike 25 deg, dip 61 deg.')
    ..writeln(
      '- Active cells: X=5..11 and W=1..4; Mark=0 assumed cells excluded.',
    )
    ..writeln(
      '- Rupture start: 36.6928 N, 137.8910 E, 4.59 km; '
      'centre of subfault Xorg=7, Worg=2.',
    )
    ..writeln(
  ...
  _writeDistanceRow(buffer, 'JMA active finite rectangle', faultDistances);
  buffer
    ..writeln()
