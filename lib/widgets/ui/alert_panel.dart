...
class _AlertPanelState extends State<AlertPanel> with SingleTickerProviderStateMixin {
  /// 闪烁动画控制器
  /// 用于高烈度预警时的红色闪烁效果
  ...
  Widget build(BuildContext context) {
    return Consumer<QuakeProvider>(
      builder: (context, provider, child) {
        ...
        final double elapsed = NtpService().now.difference(event.originTime).inMilliseconds / 1000.0;
        final double sArrival = distance / 3.5;
        final int countdown = (sArrival - elapsed).floor();

        if (countdown < -60) return const SizedBox.shrink();
            ...
            builder: (context, child) {
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: isSerious 
                    ? Colors.red.withOpacity(_flashController.value * 0.8)
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        const Text("横波预计 ", style: TextStyle(fontSize: 20, color: Colors.white70)),
                        const SizedBox(width: 10),
                        _buildCountdownText(countdown, themeColor),
                        const SizedBox(width: 10),
                        const Text(" 秒后抵达", style: TextStyle(fontSize: 20, color: Colors.white70)),
                      ],
                    ),
