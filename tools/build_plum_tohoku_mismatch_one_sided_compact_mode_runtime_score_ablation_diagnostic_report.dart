...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_mismatch_one_sided_compact_mode_runtime_score_ablation_diagnostic/report.json';
const _defaultMarkdownPath =
    'docs/baselines/plum_tohoku_mismatch_one_sided_compact_mode_runtime_score_ablation_diagnostic.generated.md';
...
void main(List<String> args) {
  ...
  stdout.writeln(
    'wrote PLUM Tohoku mismatch/one_sided/compact mode runtime-score ablation diagnostic report',
  );
...
buildPlumTohokuMismatchOneSidedCompactModeRuntimeScoreAblationDiagnosticJson({
  ...
  for (final variant in _variantOrder) {
    final validationScores = [
      for (final sample in validationTargetModes)
        sample.scoreForVariant(variant),
    ];
    final anchors = _ScoreAnchors.fromScores(validationScores);
    variants[variant] = {
      'scoreDefinition': _scoreDefinitionForVariant(variant),
      'scoreAnchors': anchors.toJson(),
      'modeScoreMeans': {
        'validationNearThresholdPositive': _ScoreMeanAccumulator.fromSamples(
          validationTargetModes.where(
            (sample) => sample.isNearThresholdPositiveMode,
          ),
          variant: variant,
        ).toJson(),
        'validationFalseMiddleTransition': _ScoreMeanAccumulator.fromSamples(
          validationTargetModes.where(
            (sample) => sample.isFalseMiddleTransitionMode,
          ),
          variant: variant,
        ).toJson(),
        'remainderNearThresholdPositive': _ScoreMeanAccumulator.fromSamples(
          remainderTargetModes.where(
            (sample) => sample.isNearThresholdPositiveMode,
          ),
          variant: variant,
        ).toJson(),
        'remainderFalseMiddleTransition': _ScoreMeanAccumulator.fromSamples(
          remainderTargetModes.where(
            (sample) => sample.isFalseMiddleTransitionMode,
          ),
          variant: variant,
      ...
      'validationAnchoredBandTransfer': {
        ...
        'remainder': _buildBandRows(
          remainderTargetModes,
          anchors,
          variant: variant,
        ),
      },
      'remainderQuartiles': _buildRemainderQuartiles(
        remainderTargetModes,
        variant: variant,
      ),
      'remainderMonotonicity': _buildRemainderMonotonicity(
        remainderTargetModes,
        variant: variant,
      ),
  ...
  return {
    ...
    'labelDefinition': const {
      ...
      'productionFeatureBoundary':
          'ablation variants must use only runtime-visible local mismatch, support geometry, evidence concentration, and evidence proximity',
    },
    'coverage': {
...
Map<String, Object?> _scoreDefinitionForVariant(String variant) {
  switch (variant) {
    ...
    case 'support_geometry_plus_proximity':
      return {
        'weights': const {'supportGeometry': 0.70, 'evidenceProximity': 0.30},
        'components': {
          'supportGeometry': 'same as support_geometry_only',
          'evidenceProximity':
...
Map<String, Object?> _buildVariantSummary(
  List<_ScoreSample> validationSamples,
  List<_ScoreSample> remainderSamples,
  _ScoreAnchors anchors, {
  required String variant,
}) {
  final validationNearMean = _mean(
    validationSamples
        .where((sample) => sample.isNearThresholdPositiveMode)
        .map((sample) => sample.scoreForVariant(variant)),
  );
  final validationFalseMean = _mean(
    validationSamples
        .where((sample) => sample.isFalseMiddleTransitionMode)
        .map((sample) => sample.scoreForVariant(variant)),
  );
  final remainderNearMean = _mean(
    remainderSamples
        .where((sample) => sample.isNearThresholdPositiveMode)
        .map((sample) => sample.scoreForVariant(variant)),
  );
  final remainderFalseMean = _mean(
    remainderSamples
        .where((sample) => sample.isFalseMiddleTransitionMode)
        .map((sample) => sample.scoreForVariant(variant)),
  );
  final bandRows = _buildBandRows(remainderSamples, anchors, variant: variant);
  final lowBand = _map(
    bandRows.firstWhere(
  ...
  final monotonicity = _buildRemainderMonotonicity(
    remainderSamples,
    variant: variant,
  );
  return {
    'validationMeanGap': validationNearMean - validationFalseMean,
    'remainderMeanGap': remainderNearMean - remainderFalseMean,
    ...
    'highToLowPositiveLift': _number(monotonicity['highToLowPositiveLift']),
    'highToLowFalseRatio': _number(monotonicity['lowToHighNegativeReduction']),
  };
}
...
List<Map<String, Object?>> _buildRemainderQuartiles(
  List<_ScoreSample> samples, {
  required String variant,
}) {
  ...
  final sorted = [...samples]
    ..sort(
      (left, right) => left
          .scoreForVariant(variant)
          .compareTo(right.scoreForVariant(variant)),
    );
...
Map<String, Object?> _buildRemainderMonotonicity(
  List<_ScoreSample> samples, {
  required String variant,
}) {
  final quartiles = _buildRemainderQuartiles(samples, variant: variant);
  ...
  final low = quartiles.isEmpty
      ? const <String, Object?>{}
      : _map(quartiles.first);
  final high = quartiles.isEmpty
      ? const <String, Object?>{}
      : _map(quartiles.last);
  ...
  return {
    ...
    'lowToHighNegativeReduction': lowNegative == 0
        ? 0.0
        : highNegative / lowNegative,
...
plumTohokuMismatchOneSidedCompactModeRuntimeScoreAblationDiagnosticMarkdown(
  ...
  final variants = _map(report['variantComparison']);
  ...
  for (final variant in _variantOrder) {
    final summary = _map(_map(variants[variant])['summary']);
    buffer.writeln(
      '| `$variant` | ${_fmt(summary['validationMeanGap'])} | ${_fmt(summary['remainderMeanGap'])} | ${_pct(summary['lowBandFalseCaptureShare'])} | ${_pct(summary['lowBandNearCaptureShare'])} | ${_pct(summary['highBandNearRate'])} | ${_pct(summary['highBandFalseRate'])} | ${_fmtMultiplier(_number(summary['highToLowPositiveLift']))} | ${_fmtMultiplier(_number(summary['highToLowFalseRatio']))} |',
    );
  }

  for (final variant in _variantOrder) {
    final variantJson = _map(variants[variant]);
    final definition = _map(variantJson['scoreDefinition']);
    final anchors = _map(variantJson['scoreAnchors']);
    final means = _map(variantJson['modeScoreMeans']);
    final transfer = _map(variantJson['validationAnchoredBandTransfer']);
    final validationBands = _list(transfer['validation']);
    final remainderBands = _list(transfer['remainder']);
    final quartiles = _list(variantJson['remainderQuartiles']);
    final monotonicity = _map(variantJson['remainderMonotonicity']);
    buffer
      ..writeln()
      ..writeln('## `$variant`')
      ..writeln()
      ..writeln(
        '- Weights: `${const JsonEncoder().convert(_map(definition['weights']))}`',
      )
      ..writeln(
    ...
    for (final mode in const [
      ...
      final row = _map(means[mode]);
      buffer.writeln('| `$mode` | ${row['count']} | ${_fmt(row['score'])} |');
    }
    buffer
      ..writeln()
    ...
    buffer
      ...
      ..writeln(
        '- High/low false-rate ratio: `${_fmtMultiplier(_number(monotonicity['lowToHighNegativeReduction']))}`.',
      );
  }
  buffer
    ..writeln()
...
class _ScoreMeanAccumulator {
  ...
  factory _ScoreMeanAccumulator.fromSamples(
    Iterable<_ScoreSample> samples, {
    required String variant,
  }) {
    final accumulator = _ScoreMeanAccumulator();
    for (final sample in samples) {
      accumulator.add(sample.scoreForVariant(variant));
    }
    return accumulator;
  }
