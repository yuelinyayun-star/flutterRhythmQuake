...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_focus_distance_site_frozen_diagnostic/report.json';
...
void main(List<String> args) {
  ...
  markdown.writeAsStringSync(
    plumTohokuFocusDistanceSiteFrozenDiagnosticMarkdown(report),
  );

  stdout.writeln(
    'wrote PLUM Tohoku focus distance/site frozen diagnostic report',
  );
...
Map<String, Object?> buildPlumTohokuFocusDistanceSiteFrozenDiagnosticJson({
...
String plumTohokuFocusDistanceSiteFrozenDiagnosticMarkdown(
  ...
  final buffer = StringBuffer()
    ..writeln('# PLUM Tohoku Focus Distance/Site Frozen Diagnostic')
    ..writeln()
  ...
  for (final threshold in _thresholds) {
    ...
    buffer
      ...
      ..writeln()
      ..writeln('### Marginal Distance Precision')
      ..writeln()
      ..writeln('| Distance | Validation P+/P | Test P+/P | Delta |')
      ..writeln('| --- | ---: | ---: | ---: |');
    final validationDistances = {
      for (final raw in _list(validation['marginalDistance']))
        _map(raw)['distance'] as String: _map(raw),
    };
    final testDistances = {
      for (final raw in _list(test['marginalDistance']))
        _map(raw)['distance'] as String: _map(raw),
    };
    for (final distance in _distanceBands) {
      final left = validationDistances[distance] ?? const {};
      final right = testDistances[distance] ?? const {};
      buffer.writeln(
        '| `$distance` | ${left['predictedPositive'] ?? 0} / ${_pct(left['precision'])} | '
  ...
  buffer
    ...
    ..writeln()
    ..writeln(
      '- Diagnostic-only. This report checks whether finer '
      'offshore-distance/site geography transfers inside the `tohoku` hotspot.',
    )
    ..writeln();
...
class _ThresholdAccumulator {
  ...
  final jointBuckets = <String, _DistanceSiteBucket>{};
  final marginalDistance = <String, _PredictedBucket>{};

  void add(_DistanceSiteSample sample, double threshold) {
    focusSampleCount++;
  ...
  String _bandFor(_DistanceSiteBucket bucket) {
...
class _DistanceSiteSample {
  final double actual;
  final double baselineRaw;
  final String distance;
  final String site;

  const _DistanceSiteSample({
...
class _BandAccumulator {
  ...
  void addBucket(_DistanceSiteBucket bucket) {
    bucketCount++;
    predictedPositive += bucket.predictedPositive;
    truePositive += bucket.truePositive;
    falsePositive += bucket.falsePositive;
