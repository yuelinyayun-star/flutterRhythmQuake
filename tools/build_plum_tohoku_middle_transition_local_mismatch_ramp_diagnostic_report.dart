...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_middle_transition_local_mismatch_ramp_diagnostic/report.json';
const _defaultMarkdownPath =
    'docs/baselines/plum_tohoku_middle_transition_local_mismatch_ramp_diagnostic.generated.md';
...
void main(List<String> args) {
  ...
  final report = buildPlumTohokuMiddleTransitionLocalMismatchRampDiagnosticJson(
  ...
  stdout.writeln(
    'wrote PLUM Tohoku middle-transition local-mismatch-ramp diagnostic report',
  );
...
Map<String, Object?> buildPlumTohokuMiddleTransitionLocalMismatchRampDiagnosticJson({
  ...
  final baseAnchors = _ScoreAnchors.fromScores(
    validation.samples.map((sample) => sample.runtimeScore).toList(),
  );
  final adjustedAnchors = _ScoreAnchors.fromScores(
    validation.samples.map((sample) => sample.adjustedScore).toList(),
  );
  ...
  return {
    ...
    'scoreDefinition': {
      'baseWeights': _scoreWeights,
      'localMismatchRamp':
          'clamp((localBelowThresholdShare10Km - 0.25) / 0.25, 0, 1)',
      'adjustedScore': 'runtimeScore * localMismatchRamp',
    },
    'labelDefinition': const {
      ...
      'productionFeatureBoundary':
          'only runtime-visible support concentration, spread variance, PLUM margin, and localBelowThresholdShare10Km are used',
    },
    'coverage': {
    ...
    'scoreAnchors': {
      'base': baseAnchors.toJson(),
      'adjusted': adjustedAnchors.toJson(),
    },
    'testOutcomeMeans': {
      'base': _buildOutcomeMeans(test, adjusted: false),
      'adjusted': _buildOutcomeMeans(test, adjusted: true),
    },
    ...
    'testDeciles': {
      'base': _buildTestDeciles(test, adjusted: false),
      'adjusted': _buildTestDeciles(test, adjusted: true),
    },
    'monotonicitySummary': {
      'base': _buildMonotonicitySummary(test, adjusted: false),
      'adjusted': _buildMonotonicitySummary(test, adjusted: true),
    },
    'comparisonSummary': _buildComparisonSummary(test),
    'errors': errors,
  };
}

Map<String, Object?> _buildComparisonSummary(_SplitAccumulator test) {
  final base = _buildMonotonicitySummary(test, adjusted: false);
  final adjusted = _buildMonotonicitySummary(test, adjusted: true);
  return {
    'baseTopToBottomLift': base['topToBottomLift'],
    'adjustedTopToBottomLift': adjusted['topToBottomLift'],
    'liftDelta':
        _number(adjusted['topToBottomLift']) - _number(base['topToBottomLift']),
    'baseDecreasingPairs': base['decreasingAdjacentPairs'],
    'adjustedDecreasingPairs': adjusted['decreasingAdjacentPairs'],
    'decreasingPairDelta':
        _number(base['decreasingAdjacentPairs']) -
        _number(adjusted['decreasingAdjacentPairs']),
  };
}

Map<String, Object?> _buildOutcomeMeans(
  ...
  for (final sample in split.samples) {
    if (sample.isMiddleTransitionZone) {
      positive.add(sample, adjusted: adjusted);
    } else {
      negative.add(sample, adjusted: adjusted);
    }
  }
  return {
...
List<Map<String, Object?>> _buildBandRows(
  ...
  for (final sample in split.samples) {
    final score = adjusted ? sample.adjustedScore : sample.runtimeScore;
    rows[anchors.bandFor(score)]!.add(sample);
  }
...
List<Map<String, Object?>> _buildTestDeciles(
  _SplitAccumulator test, {
  required bool adjusted,
}) {
  if (test.samples.isEmpty) return const [];
  final sorted = [...test.samples]
    ..sort((left, right) {
      final leftScore = adjusted ? left.adjustedScore : left.runtimeScore;
      final rightScore = adjusted ? right.adjustedScore : right.runtimeScore;
  ...
  for (var decile = 0; decile < 10; decile++) {
    ...
    final firstScore = adjusted ? bucket.first.adjustedScore : bucket.first.runtimeScore;
    final lastScore = adjusted ? bucket.last.adjustedScore : bucket.last.runtimeScore;
    rows.add({
      'decile': decile + 1,
      'scoreMin': firstScore,
      'scoreMax': lastScore,
...
Map<String, Object?> _buildMonotonicitySummary(
  _SplitAccumulator test, {
  required bool adjusted,
}) {
  final deciles = _buildTestDeciles(test, adjusted: adjusted);
...
String plumTohokuMiddleTransitionLocalMismatchRampDiagnosticMarkdown(
  ...
  final outcomeMeans = _map(report['testOutcomeMeans']);
  final transfer = _map(report['validationAnchoredBandTransfer']);
  final testDeciles = _map(report['testDeciles']);
  final monotonicity = _map(report['monotonicitySummary']);
  final comparison = _map(report['comparisonSummary']);
  final buffer = StringBuffer()
    ..writeln('# PLUM Tohoku Middle-Transition Local-Mismatch-Ramp Diagnostic')
    ..writeln()
  ...
  buffer
    ...
    ..writeln()
    ..writeln(
      '- Base weights: concentration `${_fmt(_map(scoreDefinition['baseWeights'])['concentration'])}`, '
      'spreadVariance `${_fmt(_map(scoreDefinition['baseWeights'])['spreadVariance'])}`, '
      'plumMargin `${_fmt(_map(scoreDefinition['baseWeights'])['plumMargin'])}`.',
    )
    ..writeln(
      '- Local mismatch ramp: `${scoreDefinition['localMismatchRamp']}`.',
    )
    ..writeln(
      '- Adjusted score: `${scoreDefinition['adjustedScore']}`.',
    )
    ..writeln()
    ..writeln('## Test Outcome Means')
    ..writeln()
    ..writeln(
      '| Variant | Outcome | Local Ramp | Score |',
    )
    ..writeln('| --- | --- | ---: | ---: |');
  for (final variant in const ['base', 'adjusted']) {
    final rows = _map(outcomeMeans[variant]);
    for (final outcome in const ['middleTransitionZone', 'otherFocusSamples']) {
      final row = _map(rows[outcome]);
      final value = variant == 'base' ? row['runtimeScore'] : row['adjustedScore'];
      buffer.writeln(
        '| `$variant` | `$outcome` | ${_fmt(row['localMismatchRamp'])} | ${_fmt(value)} |',
      );
    }
  }
  buffer
    ...
    ..writeln()
    ..writeln(
      '- Base top-to-bottom lift: `${_fmtMultiplier(_number(comparison['baseTopToBottomLift']))}`; adjusted: `${_fmtMultiplier(_number(comparison['adjustedTopToBottomLift']))}`.',
    )
    ..writeln(
      '- Lift delta: `${_signedDecimal(_number(comparison['liftDelta']))}`.',
    )
    ..writeln(
      '- Base decreasing pairs: `${comparison['baseDecreasingPairs']}`; adjusted: `${comparison['adjustedDecreasingPairs']}`.',
    )
    ..writeln(
      '- Decreasing-pair improvement: `${_signedDecimal(_number(comparison['decreasingPairDelta']))}`.',
    );

  for (final variant in const ['base', 'adjusted']) {
    final variantTransfer = _map(transfer[variant]);
    final validationBands = _list(variantTransfer['validation']);
    final testBands = _list(variantTransfer['test']);
    buffer
      ..writeln()
      ..writeln('## `$variant` Validation-Anchored Band Transfer')
      ..writeln()
      ..writeln(
    ...
    final variantDeciles = _list(_map(testDeciles[variant])['rows'] ?? testDeciles[variant]);
    buffer
      ..writeln()
      ..writeln('## `$variant` Test Deciles')
      ..writeln()
      ..writeln(
        '| Decile | Score Min | Score Max | Samples | Middle | Rate | Capture Share |',
      )
      ..writeln('| --- | ---: | ---: | ---: | ---: | ---: | ---: |');
    for (final raw in variantDeciles) {
      final row = _map(raw);
      buffer.writeln(
    ...
    final variantMonotonicity = _map(monotonicity[variant]);
    buffer
      ..writeln()
      ..writeln('## `$variant` Monotonicity')
      ..writeln()
      ..writeln(
        '- Top decile rate: `${_pct(variantMonotonicity['topDecileRate'])}`; bottom decile rate: `${_pct(variantMonotonicity['bottomDecileRate'])}`.',
      )
      ..writeln(
        '- Top-to-bottom lift: `${_fmtMultiplier(_number(variantMonotonicity['topToBottomLift']))}`.',
      )
      ..writeln(
        '- Non-decreasing adjacent pairs: `${variantMonotonicity['nonDecreasingAdjacentPairs']}`; decreasing pairs: `${variantMonotonicity['decreasingAdjacentPairs']}`.',
      );
  }
  ...
  buffer
    ...
    ..writeln()
    ..writeln(
      '- This report remains diagnostic-only. It checks whether multiplying the '
      'runtime score by a local-mismatch ramp materially improves ordering of '
      'the `shindo4` middle-transition regime.',
    )
    ..writeln(
...
class _ScoreMeanAccumulator {
  int count = 0;
  double localMismatchRamp = 0.0;
  double runtimeScore = 0.0;
  double adjustedScore = 0.0;

  void add(_Sample sample, {required bool adjusted}) {
    count++;
    localMismatchRamp += sample.localMismatchRamp;
  ...
  Map<String, Object?> toJson() => {
        'count': count,
        'localMismatchRamp': count == 0 ? 0.0 : localMismatchRamp / count,
        'runtimeScore': count == 0 ? 0.0 : runtimeScore / count,
        'adjustedScore': count == 0 ? 0.0 : adjustedScore / count,
      };
}
...
class _Sample {
  ...
  double get localMismatchRamp => _clamp01(
        (localBelowThresholdShare10Km - 0.25) / 0.25,
      );

  double get adjustedScore => runtimeScore * localMismatchRamp;
}
...
Map<String, Object?> _emptyReport({
  ...
  required String modelPath,
        ...
        'scoreAnchors': const {
          'base': {'q25': 0.0, 'q50': 0.0, 'q75': 0.0},
          'adjusted': {'q25': 0.0, 'q50': 0.0, 'q75': 0.0},
        },
        'testOutcomeMeans': const {},
        'validationAnchoredBandTransfer': const {},
        'testDeciles': const {},
        'monotonicitySummary': const {},
        'comparisonSummary': const {},
        'errors': errors,
      };
