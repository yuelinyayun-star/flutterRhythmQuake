...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_mismatch_one_sided_compact_support_geometry_broader_verification_diagnostic/report.json';
const _defaultMarkdownPath =
    'docs/baselines/plum_tohoku_mismatch_one_sided_compact_support_geometry_broader_verification_diagnostic.generated.md';
...
void main(List<String> args) {
  ...
  stdout.writeln(
    'wrote PLUM Tohoku mismatch/one_sided/compact support-geometry broader verification diagnostic report',
  );
...
buildPlumTohokuMismatchOneSidedCompactSupportGeometryBroaderVerificationDiagnosticJson({
  ...
  final slices = <Map<String, Object?>>[
    _buildSliceReport(
      ...
      sliceCategory: 'pooled_remainder',
      samples: remainderTargetModes,
      anchors: anchors,
      includedEventIds: eventEntries.map((entry) => entry.key).toList(),
      excludedEventIds: const [],
    ),
  ];

  for (final entry in eventEntries) {
    final pooledWithoutEvent = [
      for (final sample in remainderTargetModes)
        if (sample.eventId != entry.key) sample,
    ];
    slices.add(
      _buildSliceReport(
        sliceId: 'leave_event_out::${entry.key}',
        label: 'leave-event-out pooled remainder',
        sliceCategory: 'leave_event_out_pool',
        samples: pooledWithoutEvent,
        anchors: anchors,
        includedEventIds: [
          for (final other in eventEntries)
            if (other.key != entry.key) other.key,
        ],
        excludedEventIds: [entry.key],
      ),
    );
    ...
    slices.add(
      _buildSliceReport(
        sliceId: 'event_only::${entry.key}',
        label: 'event-only slice',
        sliceCategory: 'event_only',
        samples: entry.value,
        anchors: anchors,
        includedEventIds: [entry.key],
        excludedEventIds: const [],
      ),
    );
  }
  ...
  return {
    ...
    'coverage': {
      ...
      'eventOnlySliceCount': eventOnlySliceCount,
    ...
    'sliceCategorySummary': categorySummary,
    'sliceReports': slices,
    'errors': errors,
  };
}
...
Map<String, Object?> _buildCategorySummary(List<Map<String, Object?>> slices) {
  final byCategory = <String, Map<String, int>>{};
  for (final raw in slices) {
    final slice = _map(raw);
    final category = slice['sliceCategory']?.toString() ?? 'unknown';
    final stability = slice['stabilityLabel']?.toString() ?? 'unknown';
    final bucket = byCategory.putIfAbsent(category, () => <String, int>{});
    bucket[stability] = (bucket[stability] ?? 0) + 1;
...
plumTohokuMismatchOneSidedCompactSupportGeometryBroaderVerificationDiagnosticMarkdown(
  ...
  final categorySummary = _map(report['sliceCategorySummary']);
  ...
  final buffer = StringBuffer()
    ...
    ..writeln('| `remainder events` | ${coverage['remainderEventCount']} |')
    ..writeln('| `evaluated slices` | ${coverage['evaluatedSliceCount']} |')
    ..writeln(
      '| `leave-event-out pooled slices` | ${coverage['leaveEventOutSliceCount']} |',
    )
    ..writeln('| `event-only slices` | ${coverage['eventOnlySliceCount']} |')
    ..writeln()
    ...
    ..writeln()
    ..writeln('## Slice Category Summary')
    ..writeln()
    ..writeln(
  ...
  for (final entry in categorySummary.entries) {
    ...
    buffer.writeln(
      '| `${entry.key}` | ${row['total']} | ${row['stable']} | ${row['directional_but_noisy']} | ${row['sample_sparse']} | ${row['not_stable']} |',
    );
  }
  buffer
    ..writeln()
  ...
  for (final raw in slices) {
    ...
    buffer.writeln(
      '| `${row['sliceId']}` | `${row['sliceCategory']}` | `${row['label']}` | ${row['count']} | ${row['nearCount']} | ${row['falseCount']} | ${_fmt(row['meanGap'])} | `${row['stabilityLabel']}` | ${_pct(lowBand['negativeCaptureShare'])} | ${_pct(lowBand['positiveCaptureShare'])} | ${_pct(highBand['positiveRate'])} | ${_pct(highBand['negativeRate'])} | ${_fmtMultiplier(_number(monotonicity['highToLowPositiveLift']))} | ${_fmtMultiplier(_number(monotonicity['highToLowFalseRatio']))} |',
    );
  }
  ...
  for (final raw in slices) {
    ...
    buffer
      ...
      ..writeln()
      ..writeln(
        '- Category: `${row['sliceCategory']}`; label: `${row['label']}`; stability: `${row['stabilityLabel']}`.',
      )
      ..writeln(
        '- Included events: `${_eventListText(row['includedEventIds'])}`; excluded events: `${_eventListText(row['excludedEventIds'])}`.',
      )
      ..writeln(
...
String _eventListText(Object? raw) {
  final values = [
