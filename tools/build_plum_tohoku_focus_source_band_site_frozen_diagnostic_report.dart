...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_focus_source_band_site_frozen_diagnostic/report.json';
...
void main(List<String> args) {
  ...
  final report = buildPlumTohokuFocusSourceBandSiteFrozenDiagnosticJson(
  ...
  markdown.writeAsStringSync(
    plumTohokuFocusSourceBandSiteFrozenDiagnosticMarkdown(report),
  );

  stdout.writeln(
    'wrote PLUM Tohoku focus source-band/site frozen diagnostic report',
  );
...
Map<String, Object?> buildPlumTohokuFocusSourceBandSiteFrozenDiagnosticJson({
...
String plumTohokuFocusSourceBandSiteFrozenDiagnosticMarkdown(
  ...
  final buffer = StringBuffer()
    ..writeln('# PLUM Tohoku Focus Source-Band/Site Frozen Diagnostic')
    ..writeln()
  ...
  for (final threshold in _thresholds) {
    ...
    buffer
      ..writeln()
      ..writeln('### Source-Band/Site Buckets')
      ..writeln()
      ..writeln(
  ...
  buffer
    ...
    ..writeln()
    ..writeln(
      '- Diagnostic-only. This report checks whether finer Tohoku source '
      'sub-bands transfer inside the hotspot before adding new calibration logic.',
    )
    ..writeln();
...
class _ThresholdAccumulator {
  ...
  final jointBuckets = <String, _SourceBandSiteBucket>{};
  final marginalSourceBand = <String, _PredictedBucket>{};

  void add(_SourceBandSiteSample sample, double threshold) {
    focusSampleCount++;
  ...
  String _bandFor(_SourceBandSiteBucket bucket) {
...
class _SourceBandSiteSample {
  final double actual;
  final double baselineRaw;
  final String sourceBand;
  final String site;

  const _SourceBandSiteSample({
...
class _BandAccumulator {
  ...
  void addBucket(_SourceBandSiteBucket bucket) {
    bucketCount++;
    predictedPositive += bucket.predictedPositive;
    truePositive += bucket.truePositive;
    falsePositive += bucket.falsePositive;
