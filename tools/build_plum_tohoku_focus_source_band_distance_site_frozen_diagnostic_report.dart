...
const _defaultOutputPath =
    '.dart_tool/plum_tohoku_focus_source_band_distance_site_frozen_diagnostic/report.json';
...
const _sourceBands = ['south_tohoku', 'mid_tohoku', 'north_tohoku'];
const _distanceBands = [
  '000_030km',
  '030_060km',
  '060_100km',
  '100_200km',
...
void main(List<String> args) {
  ...
  final report = buildPlumTohokuFocusSourceBandDistanceSiteFrozenDiagnosticJson(
  ...
  markdown.writeAsStringSync(
    plumTohokuFocusSourceBandDistanceSiteFrozenDiagnosticMarkdown(report),
  );

  stdout.writeln(
    'wrote PLUM Tohoku focus source-band distance/site frozen diagnostic report',
  );
...
Map<String, Object?>
buildPlumTohokuFocusSourceBandDistanceSiteFrozenDiagnosticJson({
  ...
  for (final splitName in const ['validation', 'test']) {
    ...
    for (final rawEvent in _list(dataset['events'])) {
      ...
      for (final variant in event.variants) {
        ...
        final sourceBand = _tohokuSourceBand(estimate.latitude);
        for (final station in event.stations) {
          ...
          final sourceDistanceKm = QuakeCalculator.haversineDistance(
            estimate.latitude,
            estimate.longitude,
            station.latitude,
            station.longitude,
          ...
          for (final threshold in _thresholds) {
            ...
            splitAccumulator
                ...
                .add(
                  _SourceBandDistanceSiteSample(
                    actual: station.intensity,
                    baselineRaw: baselineRaw,
                    sourceBand: sourceBand,
                    distance: _distanceBand(sourceDistanceKm),
  ...
  return {
    'schemaVersion':
        'plum_tohoku_focus_source_band_distance_site_frozen_diagnostic_v1',
...
String plumTohokuFocusSourceBandDistanceSiteFrozenDiagnosticMarkdown(
  ...
  final buffer = StringBuffer()
    ..writeln('# PLUM Tohoku Focus Source-Band Distance/Site Frozen Diagnostic')
    ..writeln()
  ...
  for (final threshold in _thresholds) {
    ...
    buffer
      ...
      ..writeln()
      ..writeln('### Marginal Source-Band Precision')
      ..writeln()
      ..writeln('| Source Band | Validation P+/P | Test P+/P | Delta |')
      ..writeln('| --- | ---: | ---: | ---: |');
    final validationBands = {
      for (final raw in _list(validation['marginalSourceBand']))
        _map(raw)['sourceBand'] as String: _map(raw),
    };
    final testBands = {
      for (final raw in _list(test['marginalSourceBand']))
        _map(raw)['sourceBand'] as String: _map(raw),
    };
    for (final sourceBand in _sourceBands) {
      final left = validationBands[sourceBand] ?? const {};
      final right = testBands[sourceBand] ?? const {};
      buffer.writeln(
        '| `$sourceBand` | ${left['predictedPositive'] ?? 0} / ${_pct(left['precision'])} | '
    ...
    buffer
      ..writeln()
      ..writeln('### Source-Band Distance/Site Buckets')
      ..writeln()
      ..writeln(
    ...
    final validationBuckets = {
      for (final rawBucket in _list(validation['jointBuckets']))
        '${_map(rawBucket)['sourceBand']}|${_map(rawBucket)['distance']}|${_map(rawBucket)['site']}':
            _map(rawBucket),
    };
    final testBuckets = {
    ...
    for (final key in orderedKeys) {
      ...
      buffer.writeln(
        '| `${left['sourceBand']}` | `${left['distance']}` | `${left['site']}` | '
    ...
    buffer
      ...
      ..writeln('| --- | ---: | ---: | ---: | ---: |');
    final validationBandSummary = _map(validation['bandSummary']);
    final testBandSummary = _map(test['bandTransferSummary']);
    for (final band in const ['high', 'medium', 'low', 'insufficient']) {
      final left = _map(validationBandSummary[band]);
      final right = _map(testBandSummary[band]);
      buffer.writeln(
  ...
  buffer
    ...
    ..writeln()
    ..writeln(
      '- Diagnostic-only. This report checks whether combining Tohoku source '
      'sub-bands with offshore-distance/site geography transfers inside the '
      '`tohoku` hotspot before adding new calibration logic.',
    )
    ..writeln();
...
class _ThresholdAccumulator {
  ...
  final jointBuckets = <String, _SourceBandDistanceSiteBucket>{};
  final marginalSourceBand = <String, _PredictedBucket>{};

  void add(_SourceBandDistanceSiteSample sample, double threshold) {
    focusSampleCount++;
    final actualPositive = sample.actual >= threshold;
    baseline.add(actualPositive: actualPositive);
    final jointKey = '${sample.sourceBand}|${sample.distance}|${sample.site}';
    jointBuckets
        .putIfAbsent(jointKey, _SourceBandDistanceSiteBucket.new)
        .add(
    ...
    return {
      ...
      'marginalSourceBand': [
        for (final sourceBand in _sourceBands)
          {
            'sourceBand': sourceBand,
            ...?marginalSourceBand[sourceBand]?.toJson(),
          },
      ],
  ...
  String _bandFor(_SourceBandDistanceSiteBucket bucket) {
...
class _SourceBandDistanceSiteSample {
  final double actual;
  final double baselineRaw;
  final String sourceBand;
  final String distance;
  final String site;

  const _SourceBandDistanceSiteSample({
    required this.actual,
    required this.baselineRaw,
    required this.sourceBand,
    required this.distance,
    required this.site,
  });
}

class _SourceBandDistanceSiteBucket {
  String sourceBand = '';
  String distance = '';
  String site = '';
...
class _BandAccumulator {
  ...
  void addBucket(_SourceBandDistanceSiteBucket bucket) {
    bucketCount++;
    predictedPositive += bucket.predictedPositive;
    truePositive += bucket.truePositive;
    falsePositive += bucket.falsePositive;
...
String? _tohokuSourceBand(double latitude) {
  if (latitude < 37.5 || latitude >= 41) return null;
  if (latitude < 38.5) return 'south_tohoku';
  if (latitude < 39.5) return 'mid_tohoku';
  return 'north_tohoku';
}

String _distanceBand(double distanceKm) {
