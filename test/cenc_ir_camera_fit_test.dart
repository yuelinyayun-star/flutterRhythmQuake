...
// Unmodified response from the production endpoint:
// https://api-cencint-public.nowquake.cn/event/20260914170727
...
Rect screenBounds(MapCamera camera) {
...
void main() {
  ...
  for (final size in [
    ...
    testWidgets('raw Mojiang stations fill the visible desktop area $size', (
      tester,
    ) async {
      ...
      await finishMove();
      final before = screenBounds(controller.camera);
      ...
      await finishMove();
      final bounds = screenBounds(controller.camera);
      final visible = padding.deflateRect(Offset.zero & size);
      expect(data.instrumentIntensities, hasLength(143));
      expect(points.length, greaterThan(100));
      for (final point in points) {
        final offset =
            controller.camera.projectAtZoom(point) -
            controller.camera.pixelOrigin;
        expect(visible.inflate(1).contains(offset), isTrue, reason: '$point');
      }
      expect((bounds.center - visible.center).distance, lessThan(1));
      expect(
        math.max(bounds.width / visible.width, bounds.height / visible.height),
        closeTo(1, 0.01),
      );
      expect(bounds.height, greaterThan(before.height * 2));
      debugPrint(
