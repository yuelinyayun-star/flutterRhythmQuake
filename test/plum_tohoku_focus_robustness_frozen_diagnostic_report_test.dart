...
import '../tools/build_plum_tohoku_focus_robustness_frozen_diagnostic_report.dart';

void main() {
  test(
    'PLUM Tohoku focus robustness frozen diagnostic is structured and non-production',
    () {
      final report = buildPlumTohokuFocusRobustnessFrozenDiagnosticJson();

      expect(
        report['schemaVersion'],
        'plum_tohoku_focus_robustness_frozen_diagnostic_v1',
      );
      ...
      for (final label in <String>['shindo4', 'shindo5-']) {
        ...
        if (label == 'shindo4') {
          expect(validationBaseline['precision'], isA<double>());
          expect(testBaseline['precision'], isA<double>());
          expect(
      ...
      final markdown = plumTohokuFocusRobustnessFrozenDiagnosticMarkdown(report);
      expect(
        markdown,
        contains('# PLUM Tohoku Focus Robustness Frozen Diagnostic'),
      );
      expect(markdown, contains('### Focus Baseline'));
      expect(markdown, contains('### Joint Buckets'));
