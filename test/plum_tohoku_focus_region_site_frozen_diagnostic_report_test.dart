...
import '../tools/build_plum_tohoku_focus_region_site_frozen_diagnostic_report.dart';

void main() {
  test(
    'PLUM Tohoku focus region/site frozen diagnostic is structured and non-production',
    () {
      final report = buildPlumTohokuFocusRegionSiteFrozenDiagnosticJson();

      expect(
        report['schemaVersion'],
        'plum_tohoku_focus_region_site_frozen_diagnostic_v1',
      );
      ...
      for (final label in <String>['shindo4', 'shindo5-']) {
        ...
        expect(validation['marginalSite'], isA<List>());
        expect(test['marginalSite'], isA<List>());
