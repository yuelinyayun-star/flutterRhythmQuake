...
void main() {
  test('LPGM vs NIED scale comparison', () async {
    ...
    for (int x = 200; x < w1; x++) {
      ...
      for (int y = 0; y < h1; y++) {
        final off = (y * w1 + x) * 4;
        if (raw1.getUint8(off) != 0 ||
            raw1.getUint8(off + 1) != 0 ||
            raw1.getUint8(off + 2) != 0)
          c++;
      }
      if (c > lpgmBarCount) {
        lpgmBarCount = c;
    ...
    for (int x = 0; x < w2; x++) {
      ...
      for (int y = 0; y < h2; y++) {
        final off = (y * w2 + x) * 4;
        if (raw2.getUint8(off) != 0 ||
            raw2.getUint8(off + 1) != 0 ||
            raw2.getUint8(off + 2) != 0)
          c++;
      }
      if (c > niedBarCount) {
        niedBarCount = c;
    ...
    for (double p = 0.0; p <= 1.0; p += 0.05) {
      // LPGM
      final lpgmY = (lpgmBottom - p * (lpgmBottom - lpgmTop)).round().clamp(
        lpgmTop,
        lpgmBottom,
      );
      ...
      for (int x = lpgmBarX - 5; x <= lpgmBarX + 5; x++) {
        ...
        final r = raw1.getUint8(off),
            g = raw1.getUint8(off + 1),
            b = raw1.getUint8(off + 2);
      ...
      // NIED
      final niedY = (niedBottom - p * (niedBottom - niedTop)).round().clamp(
        niedTop,
        niedBottom,
      );
      ...
      for (int x = niedBarX - 5; x <= niedBarX + 5; x++) {
        ...
        final r = raw2.getUint8(off),
            g = raw2.getUint8(off + 1),
            b = raw2.getUint8(off + 2);
        if (r == 0 && g == 0 && b == 0) continue;
        nSumR += r;
        nSumG += g;
        nSumB += b;
