...
import '../tools/build_plum_tohoku_focus_source_band_site_frozen_diagnostic_report.dart';

void main() {
  test(
    'PLUM Tohoku focus source-band/site frozen diagnostic is structured and non-production',
    () {
      final report = buildPlumTohokuFocusSourceBandSiteFrozenDiagnosticJson();

      expect(
        report['schemaVersion'],
        'plum_tohoku_focus_source_band_site_frozen_diagnostic_v1',
      );
      ...
      for (final label in <String>['shindo4', 'shindo5-']) {
        ...
        expect(validation['marginalSourceBand'], isA<List>());
        expect(test['marginalSourceBand'], isA<List>());
      ...
      final markdown = plumTohokuFocusSourceBandSiteFrozenDiagnosticMarkdown(
        report,
      );
      expect(
        markdown,
        contains('# PLUM Tohoku Focus Source-Band/Site Frozen Diagnostic'),
      );
      expect(markdown, contains('### Focus Baseline'));
      expect(markdown, contains('### Marginal Source-Band Precision'));
      expect(markdown, contains('### Source-Band/Site Buckets'));
